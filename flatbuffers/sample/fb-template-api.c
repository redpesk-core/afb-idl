...
void {{apiname}}_{{verbname}}_reply_cb(
		void *closure,
		int status,
		unsigned nreplies,
		...
		afb_req_t req)
{
	{{apiname}}_{{verbname}}_reply(req, status, nreplies >= 1 ? afb_data_addref(replies[0]) : 0);
}

void {{apiname}}_{{verbname}}_process(afb_req_t req, afb_data_t data)
{
	afb_req_subcall(req, "{{apiname}}_remote", "{{verbname}}", 1, &data, 0, {{apiname}}_{{verbname}}_reply_cb, NULL);
}
