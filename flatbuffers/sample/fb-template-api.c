		...
		afb_req_x4_t req)
{
	...
	if (nreplies != 1)
		AFB_REQ_ERROR(req, "\"{{verbname}}\" reply expected 1 argument (%d receved)", nreplies);

	else {
		rc = {{response}}_to_afb_data_j(replies[0], &dataholder);
		if (rc < 0)
			AFB_REQ_ERROR(req, "\"{{verbname}}\" reply : BAD argument");
		else {
			afb_req_reply(req, rc, 1, &dataholder);
			return;
		}
	}
	afb_req_reply(req, status, 0, 0);
}

int {{apiname}}_{{verbname}}_process(afb_data_t holder, afb_req_t req)
{
	afb_req_subcall(req, "{{apiname}}_remote", "{{verbname}}", 1, &holder, 0, {{apiname}}_{{verbname}}_process_reply_cb, NULL);
